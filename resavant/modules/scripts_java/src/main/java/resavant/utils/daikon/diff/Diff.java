...
public final class Diff {
	...
	private static String usage = UtilMDE.joinLines("Usage:", "    java daikon.diff.Diff [flags...] file1 [file2]",
	...
	// this is set only when the manip flag is set "-z", that is when treeManip
	// != null
	private static /* @MonotonicNonNull */ PptMap manip1 = null;
	private static /* @MonotonicNonNull */ PptMap manip2 = null;

	public static PrintFeaturesVisitor featureExtractionVisitor = null;
	...
	/**
	 * Comparators to sort the sets of invs, and to combine the two sets into the
	 * pair tree. Can be overriden by command-line options.
	 **/
	...
	public Diff(boolean examineAllPpts, boolean ignoreNumberedExits, /* @Nullable */
			/* @ClassGetName */ String invSortComparator1Classname, /* @Nullable */
			/* @ClassGetName */ String invSortComparator2Classname, /* @Nullable */
			/* @ClassGetName */ String invPairComparatorClassname, Comparator<Invariant> defaultComparator)
	...
	public static void main(String[] args) throws FileNotFoundException, StreamCorruptedException,
		...
		try {
			if (args.length < 3) {
				System.out.println("At least 3 parameters");
				System.exit(0);
			}
			System.out.println(args[2]);
			mainHelper(Arrays.copyOfRange(args, 0, args.length - 1), args[args.length - 1]);
	 ...
	 **/
	public static void mainHelper(final String[] args, String outputFeatureFile)
		...
		Getopt g = new Getopt("daikon.diff.Diff", args, "Hhyduastmxno:jzpevl", longOpts);
		int c;
		while ((c = g.getopt()) != -1) {
			switch (c) {
			case 0:
				...
				if (Daikon.help_SWITCH.equals(optionName)) {
					...
					if (invSortComparator1Classname != null) {
						throw new Error(
								"multiple --" + INV_SORT_COMPARATOR1_SWITCH + " classnames supplied on command line");
					}
					@SuppressWarnings("signature")
					// user input, should be checked
					/* @ClassGetName */ String cgn = g.getOptarg();
					invSortComparator1Classname = cgn;
					System.out.println("square 1 " + invSortComparator1Classname);
				} else if (INV_SORT_COMPARATOR2_SWITCH.equals(optionName)) {
					if (invSortComparator2Classname != null) {
						throw new Error(
								"multiple --" + INV_SORT_COMPARATOR2_SWITCH + " classnames supplied on command line");
					}
					@SuppressWarnings("signature")
					// user input, should be checked
					/* @ClassGetName */ String cgn = g.getOptarg();
					invSortComparator2Classname = cgn;
					System.out.println("square 2 " + invSortComparator2Classname);
				} else if (INV_PAIR_COMPARATOR_SWITCH.equals(optionName)) {
					if (invPairComparatorClassname != null) {
						throw new Error(
								"multiple --" + INV_PAIR_COMPARATOR_SWITCH + " classnames supplied on command line");
					}
					@SuppressWarnings("signature")
					// user input, should be checked
					/* @ClassGetName */ String cgn = g.getOptarg();
					invPairComparatorClassname = cgn;
					System.out.println("square 3 " + invPairComparatorClassname);
		...
		Diff diff = new Diff(examineAllPpts, ignoreNumberedExits, invSortComparator1Classname,
				invSortComparator2Classname, invPairComparatorClassname, defaultComparator);

		if ((!(diff.invSortComparator1.getClass().toString().equals(diff.invSortComparator2.getClass().toString())))
				|| (!(diff.invSortComparator1.getClass().toString()
		...
		if (numFiles == 1) {
			...
			PptMap map1 = FileIO.read_serialized_pptmap(new File(filename1), false // use
																					// saved
																					// config
			);
			PptMap map2 = FileIO.read_serialized_pptmap(new File(filename2), false // use
																					// saved
																					// config
			);
			manip1 = FileIO.read_serialized_pptmap(new File(filename3), false // use
																				// saved
																				// config
			);
			manip2 = FileIO.read_serialized_pptmap(new File(filename4), false // use
																				// saved
																				// config
			);
			...
			// RootNode pass_and_both = diff.diffPptMap (manip1, map2,
			// includeUnjustified);
			// RootNode fail_and_both = diff.diffPptMap (manip2, map2,
			// includeUnjustified);
			...
			/*
			 * PptCountVisitor mcv = new PptCountVisitor (System.out, verbose, false);
			 * 
			 * root.accept (mcv); mcv.printFinal(); System.out.println ( "Precison: " +
			 * mcv.calcPrecision()); System.out.println ( "Recall: " + mcv.calcRecall());
			 * System.out.println ("Success"); // System.exit(0);
			 */
		...
		if (printDiff) {
			{
				PrintDifferingInvariantsVisitor v = new PrintDifferingInvariantsVisitor(System.out, verbose,
						printEmptyPpts, printUninteresting);
				root.accept(v);
			}
		...
		if (printFeatures) {
			...
			featureExtractionVisitor = new PrintFeaturesVisitor(System.out, null, verbose, printEmptyPpts,
					printUninteresting);
			root.accept(featureExtractionVisitor);
			if (featureExtractionVisitor.getToPrintFeatureString().size() > 0) {
				File of = new File(outputFeatureFile);
				if (of.getParentFile() != null && of.getParentFile().isDirectory() == false) {
					of.getParentFile().mkdirs();
				}
				PrintWriter writer = new PrintWriter(new BufferedWriter(new FileWriter(outputFeatureFile)), true);
	...
	/**
	 * Extracts the PptTopLevel and Invariants out of a pptMap, and places them into
	 * an InvMap. Maps PptTopLevel to a List of Invariants. The InvMap is a cleaner
	 * representation than the PptMap, and it allows us to more easily manipulate
	 * the contents. The InvMap contains exactly the elements contained in the
	 * PptMap. Conditional program points are also added as keys. Filtering is done
	 * when creating the pair tree. The ppts in the InvMap must be sorted, but the
	 * invariants need not be sorted.
	 **/
	...
	/**
	 * Returns a pair tree of corresponding program points, and corresponding
	 * invariants at each program point. This tree can be walked to determine
	 * differences between the sets of invariants. Calls diffInvMap and asks to
	 * include all justified invariants
	 **/
	 ...
	 * Returns a pair tree of corresponding program points, and corresponding
	 * invariants at each program point. This tree can be walked to determine
	 * differences between the sets of invariants. The tree consists of the
	 * invariants in map1 and map2. If includeUnjustified is true, the unjustified
	 * invariants are included.
	 **/
	public RootNode diffInvMap(InvMap map1, InvMap map2, boolean includeUnjustified) {
		RootNode root = new RootNode();

		Iterator<Pair</* @Nullable */ PptTopLevel, /* @Nullable */ PptTopLevel>> opi = new OrderedPairIterator<PptTopLevel>(
				map1.pptSortedIterator(PPT_COMPARATOR), map2.pptSortedIterator(PPT_COMPARATOR), PPT_COMPARATOR);
		while (opi.hasNext()) {
	...
	/**
	 * Diffs two PptMaps by converting them to InvMaps. Provided for compatibiliy
	 * with legacy code. Calls diffPptMap and asks to include all invariants.
	 **/
	...
	/**
	 * Diffs two PptMaps by converting them to InvMaps. Provided for compatibiliy
	 * with legacy code. If includeUnjustified is true, the unjustified invariants
	 * are included.
	 **/
	...
	/**
	 * Returns true if the program point should be added to the tree, false
	 * otherwise.
	 **/
	...
	/**
	 * Takes a pair of corresponding top-level program points and maps, and returns
	 * a tree of the corresponding invariants. Either of the program points may be
	 * null. If includeUnjustied is true, the unjustified invariants are included.
	 **/
	private PptNode diffPptTopLevel(/* @Nullable */ PptTopLevel ppt1, /* @Nullable */ PptTopLevel ppt2, InvMap map1,
			InvMap map2, boolean includeUnjustified) {
		PptNode pptNode = new PptNode(ppt1, ppt2);

		assert ppt1 == null || ppt2 == null
				|| PPT_COMPARATOR.compare(ppt1, ppt2) == 0 : "Program points do not correspond";
		...
		Iterator<Pair</* @Nullable */ Invariant, /* @Nullable */ Invariant>> opi = new OrderedPairIterator<Invariant>(
				invs1.iterator(), invs2.iterator(), invPairComparator);
		while (opi.hasNext()) {
	...
	private List<Invariant> findCondPpt(PptMap manip, PptTopLevel ppt) {
		...
		for (String somePptName : manip.nameStringSet()) {
			...
			if (targ.equals(somePptName)) {
				...
				// map: iterating over keySet
				/* @NonNull */ PptTopLevel repl = manip.get(somePptName);
				return repl.getInvariants();
			}
		}
		// System.out.println ("Could not find the left hand side of
		// implication!!!");
	...
	private List<Invariant> findNormalPpt(PptMap manip, PptTopLevel ppt) {
		...
		// String targ = targetName.substring (0,
		// targetName.lastIndexOf(";condition"));
	 ...
	 **/
	/*
	 * @EnsuresNonNull({"invSortComparator1", "invSortComparator2",
	 * "invPairComparator"})
	 */
	...
	/**
	 * If the classname is non-null, returns the comparator named by the classname.
	 * Else, returns the default.
	 **/
	private static Comparator<Invariant> selectComparator(/* @Nullable */ /* @ClassGetName */ String classname,
			Comparator<Invariant> defaultComparator)
			throws ClassNotFoundException, InstantiationException, IllegalAccessException {
